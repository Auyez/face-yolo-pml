...
#X, Y = preload("WIDER_train/wider_face_train_bbx_gt.txt", "WIDER_train/images")
...
validation_generator = DataGenerator(X, Y, 1)
...
#model.load_weights('weights.60.hdf')
...
callback = callbacks.ModelCheckpoint('weights/weights.{epoch:02d}.hdf', verbose=0, save_best_only=False, save_weights_only=True, mode='auto', period=10)
lr = ChangeLearningRate()
t_nan = callbacks.TerminateOnNaN()
#start validation on epoch 60
validation = ValidationCallback(validation_generator, 0.5, 0.3, 300, 10)
